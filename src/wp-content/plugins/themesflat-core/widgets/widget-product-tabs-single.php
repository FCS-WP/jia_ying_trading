<?php
class TFProductTab_Widget extends \Elementor\Widget_Base {
    ...
    public function get_title() {
        return esc_html__( 'TF Product Tabs Single', 'themesflat-core' );
    }
    ...
    public function get_style_depends() {
		return ['owl-carousel','swiper-min','tf-product-single','tf-product','tf-tabs'];
	}

    public function get_script_depends() {
		return [ 'owl-carousel','swiper-min','countdown','tf-tab' ];
	}
	...
	protected function register_controls() {
		...
		$this->add_control( 'heading_image_size3',
			[
				'label' => esc_html__( 'Image Size Thumnail Slider', 'themesflat-core' ),
				'type' => \Elementor\Controls_Manager::HEADING,
			]
		);
	...
	// Start Layout        
		...
		$this->add_control( 
			'posts_per_page',
			[
				'label' => esc_html__( 'Product Per Page', 'themesflat-core' ),
	...
	// Start Style Tab Nav
		...
		$this->add_responsive_control( 'title_margin2',
			[
				'label' => esc_html__( 'Margin', 'themesflat-core' ),
				'type' => \Elementor\Controls_Manager::DIMENSIONS,
				'size_units' => ['px', 'em', '%'],
	...
	// Start General Style 
		...
		$this->add_group_control( 
			...
			[
				'name' => 'box_shadow',
				'label' => esc_html__( 'Box Shadow', 'themesflat-core' ),
				'selector' => '{{WRAPPER}} .tf-products-single .item .inner',
			]
		);
		...
		$this->add_responsive_control( 
			...
			[
				...
				'size_units' => [ 'px' , '%' ],
				'selectors' => [
					'{{WRAPPER}} .tf-products-single .item .inner' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		$this->end_controls_section();    
	...
	protected function render($instance = []) {
		...
		$this->add_render_attribute( 'tf_product_wrapper', ['id' => "tf-products-{$this->get_id()}", 'class' => ['tf-products-single', 'data-tabid' => $this->get_id() ]] );
		...
		<div <?php echo $this->get_render_attribute_string('tf_tabs_wrapper'); ?>>
			<div class="tf-tabnav">
				...
				<?php foreach ($settings['tab_list'] as $tab): $count_li ++;?>
					<li class="tab-filter tablinks <?php //echo esc_attr($tab['set_active']); ?>  ?>" data-tab="tab-<?php echo esc_attr($count_li); ?>" data-post='<?php echo $settings['posts_per_page'] ?>' data-filter='<?php echo $tab['filter_by'] ?>' >						
			...
			<div class="tf-tabcontent">
				<?php //foreach ($settings['tab_list'] as $tab): $count_content ++;?>
					...
					$query = new WP_Query( $query_args );
						if ( $query->have_posts() ) : ?>
							<div id="product-tab-content" <?php echo $this->get_render_attribute_string('tf_product_wrapper'); ?>  >
								
									<?php while ( $query->have_posts() ) : $query->the_post();						
										$attr['settings'] = $settings; ?>
										<?php 
										if ($count == 1) {
											tf_get_template_widget("product-tab-single/style1", $attr);
										}
										else tf_get_template_widget("product-tab-single/style2", $attr);
										?>			
									<?php $count ++	; if ($count >= 7) {$count = 0;} endwhile; ?>
