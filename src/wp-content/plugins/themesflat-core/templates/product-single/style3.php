...
if(\Elementor\Group_Control_Image_Size::get_attachment_image_src( $get_id_post_thumbnail, 'thumbnail2', $settings )){
    $featured_image = sprintf('<img src="%s" class="img_thumbnail" alt="image">', \Elementor\Group_Control_Image_Size::get_attachment_image_src( $get_id_post_thumbnail, 'thumbnail2', $settings )); 
} else $featured_image ='';
    ...
    <div class="image">
        <div class="swiper-container gallery-slider">
            <div class="swiper-wrapper">
                <div class="swiper-slide"><a href="<?php echo the_permalink();?>"><?php echo sprintf('%s',$featured_image)?></a><?php if($sale_price != 0) { ?><div class="price-save"><div class="label"><?php echo esc_html__( 'Save', 'themesflat-core' ) ?></div><?php echo esc_html__( '$', 'themesflat-core' ).$product_sale.esc_html__( '.000', 'themesflat-core' )  ?></div><?php } ?> <?php echo themesflat_product_action_btn();?></div>
                <?php
                ...
                if($count != 0) {
                    foreach( $attachment_ids as $attachment_id ) { 
                        ...
                        <div class="swiper-slide"><a href="<?php echo the_permalink();?>"><img src="<?php echo $image_link; ?>" alt="Image"></a><?php if($sale_price != 0) { ?><div class="price-save"><div class="label"><?php echo esc_html__( 'Save', 'themesflat-core' ) ?></div><?php echo esc_html__( '$', 'themesflat-core' ).$product_sale.esc_html__( '.000', 'themesflat-core' )  ?></div><?php } ?></div>
                    <?php $c++; if ($c > 2) break; }  }
                ?>
            </div>
        </div>
